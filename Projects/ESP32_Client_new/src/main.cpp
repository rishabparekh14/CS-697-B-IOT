...
class ClientCallbacks : public NimBLEClientCallbacks
{
  void onConnect(NimBLEClient *pClient)
  {
    ...
    /** After connection we should chan e the parameters if we don't need fast response times.
...
void checkAndBlink(std::string str)
{
  int moi = atoi(str.c_str());
  if (moi >= 80)
  {
...
void notifyCB(NimBLERemoteCharacteristic *pRemoteCharacteristic, uint8_t *pData, size_t length, bool isNotify)
{
  ...
  std::string v = std::string((char *)pData, length);
...
bool connectToServer()
{
  ...
  { /** make sure it's not null */
    ...
    { /** make sure it's not null */
      if (pChr->canRead())
      {
        Serial.print("The Moisture at: ");
        Serial.print(pChr->getUUID().toString().c_str());
        Serial.print(" is: ");
        Serial.print(pChr->readValue().c_str());
        Serial.println(" %");
        checkAndBlink(pChr->readValue().c_str());
      }
